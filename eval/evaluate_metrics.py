...
def calculate_f1score(candidate, reference):
    ...
    print(word_set)
    candidate_words_count = Counter(candidate_words)
    reference_words_count = Counter(reference_words)
...
#F1 for closed set
def extract_key_answer(candidate):
    keywords = ["yes", "no"]
    candidate = candidate.lower()
    candidate = candidate.replace("yes,", "yes")
    candidate = candidate.replace("no,", "no")
...
def calculate_metrics_for_closed_set_extracted(candidate, reference):
    extracted_answer = extract_key_answer(candidate)
    print(extracted_answer)
    if extracted_answer is None:
        return 0, 0, 0
    print("calculate_metrics_for_closed_set_extracted begin")

    tp = fp = fn = tn = 0
    print("extracted_answer : ",extracted_answer ,"reference : ",reference)
    if extracted_answer == reference:
        ...
        if extracted_answer == "yes":
            ...
            fn = 1
    print("tp : ",tp, " fn : ",fn," tp + fn : ",tp+fn," fp : ",fp," recall : ,tp / (tp + fn)")
